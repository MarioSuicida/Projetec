...
export class SalvatransacaoPage {
  ...
  enviar(form: NgForm) {
    ...
    if (!this.transacao.ID_usuario) {
      ...
      this.mostrarMensagemErro('ID do usuário não encontrado.');
      return;
    }
  
    // Extraindo dia, mês e ano da data escolhida
    const dataEscolhida = new Date(this.transacao['data']);
    this.transacao.dia = dataEscolhida.getDate().toString(); // Salvando o dia da transação
    ...
    console.log('Dia da transação:', this.transacao.dia); // Verificação do dia
    ...
    // Remover campos auxiliares antes de salvar a transação
    ...
    this.usuarioService.criar<transacao>('transacao', this.transacao).subscribe(
      ...
      (error) => {
        ...
        this.mostrarMensagemErro('Erro ao salvar a transação. Tente novamente.');
      }
    );
  }
